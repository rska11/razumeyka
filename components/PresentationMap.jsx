...
const steps = [
  ...
  },
  {
    ...
    image: '/images/before-after-progress.png',
    ...
    <section ref={sectionRef} className="presentation-slide rainbow-band text-white">
      ...
      <div className="container-pad relative">
        <div className="grid gap-10 lg:grid-cols-[0.72fr_1.28fr] lg:items-center">
          ...
          >
            <span className="inline-flex rotate-[-1deg] rounded-[8px] border-2 border-white bg-night px-4 py-2 text-xs font-extrabold uppercase tracking-[0.16em] text-white shadow-[7px_7px_0_rgba(255,255,255,0.18)]">
              путь развития
            </span>
            <h2 className="mt-6 font-display text-[2.6rem] font-bold leading-[1.02] tracking-[-0.03em] sm:text-6xl lg:text-[5rem]">
              От “не хочу” до “у меня получилось”
            </h2>
            <p className="mt-6 max-w-xl text-xl font-semibold leading-9 text-white/78">
          ...
          <div className="grid gap-5 md:grid-cols-4">
            {steps.map((step, index) => (
              <article
                key={step.title}
                className={`group relative min-h-[360px] overflow-hidden rounded-[8px] border-2 border-white/72 bg-white/18 p-4 shadow-[10px_10px_0_rgba(255,255,255,0.12)] backdrop-blur-xl transition duration-500 hover:-translate-y-3 hover:scale-[1.02] ${step.offset} ${
              ...
              >
                ...
                <div className="relative h-28 overflow-hidden rounded-[8px] border border-white/30 bg-white/20">
                  <img
                    src={step.image}
                ...
                <div className="relative mt-5 flex items-center justify-between">
                  <div className={`flex h-[52px] w-[52px] items-center justify-center rounded-[8px] bg-gradient-to-br ${step.accent} text-white shadow-luxe`}>
                    <Icon name={step.icon} className="h-7 w-7" />
                  </div>
                  <span className="font-display text-5xl font-black text-white/26">0{index + 1}</span>
                </div>

                <p className="relative mt-8 font-display text-2xl font-black leading-tight">{step.title}</p>
                ...
                {index < steps.length - 1 && (
                  <span className="absolute -right-7 top-1/2 z-10 hidden h-14 w-14 -translate-y-1/2 animate-floaty items-center justify-center rounded-full border-2 border-white bg-night text-white shadow-luxe md:flex">
